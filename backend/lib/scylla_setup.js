...
class ScyllaSetupService {
  ...
  async setup (tables, { isDev } = {}) {
    ...
    return CacheService.lock('scylla_setup1', async () => {
...
`
  ...
  addColumnToQuery (q, type, key) {
    if (typeof type === 'object') {
      try {
        if (type.type === 'json' || type.type === 'html') {
          type.type = 'text'
        }
        if (type.subType === 'json') {
          type.subType = 'text'
        }
        if (type.subType2 === 'json') {
          type.subType2 = 'text'
        }
        if (type.subType2) {
  ...
  */
  async createTableIfNotExist (table) {
    ...
    _.forEach(primaryFields, ({ key, type }) => {
    ...
    // add any new columns
    _.forEach(normalFields, ({ key, type }) => {
    ...
    await q.run().catch((err) => {
      if (err.message.indexOf('Cannot add a non counter column') === -1) {
        throw err
      }
    })
