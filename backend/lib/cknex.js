...
function cknex (keyspace) {
  ...
  instance.run = function (options = {}) {
    // cid = callerId.getData()
    const self = this
    return ready.then(() => new Promise((resolve, reject) => {
      // console.log cid
      // console.log(self._columnFamily) //, JSON.stringify(self._statements))
      // console.log ''
      // console.log '----------'
      // console.log ''
      if (options.isLwtRead) {
        options.consistency = types.consistencies.serial
      } else if (options.isLwtWrite) {
        options.serialConsistency = types.consistencies.serial
      }
      return self.exec(options, function (err, result) {
...
cknex.getTimeUuidStr = function (time) {
  if (time) {
    ...
    return `${cassandra.types.TimeUuid.fromDate(Time.dateToUTC(time))}`
  } else {
...
cknex.emptyUuid = '00000000-0000-0000-0000-000000000000'
