...
export default {
  ...
  // main thing we want to use for right now is the complex org query that contains orgUser, roles, etc...
  // we want that cached, but we also want to clear that cache (using categories) effectively
  async execQuery (query, variables, context, cacheableFragments, queryFn) {
    const parsedQuery = parse(query)
    const args = _.reduce(parsedQuery.definitions[0].selectionSet.selections[0].arguments, (obj, arg) => {
      const key = arg.name.value
      const value = variables[key] || arg.value.value
      obj[key] = value
      return obj
    }, {})
    ...
    return Cache.preferCache(cacheKey, queryFn, { expireSeconds, categoriesFn, dontCacheIfGraphqlErrors: true })
  },
